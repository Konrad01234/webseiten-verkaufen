...
function updateNav() {
  ...
  if (state.user) {
    ...
    actions.innerHTML = `
      <div class="user-menu">
        ...
        <div class="user-dropdown ${state.dropdownOpen ? '' : 'hidden'}" id="user-dropdown" role="menu">
          ...
          <a href="#" data-action="navAndToggleDropdown" data-page="${isEmployer ? 'employer-profile' : 'worker-profile'}">Profil</a>
          ${!isEmployer ? '<a href="#" data-action="navAndToggleDropdown" data-page="saved-jobs">Gespeicherte Jobs</a>' : ''}
          ${!isEmployer ? '<a href="#" data-action="navAndToggleDropdown" data-page="cv-builder">Lebenslauf</a>' : ''}
          ${isEmployer ? '<a href="#" data-action="goPostJobAndToggleDropdown">Anzeige schalten</a>' : ''}
          <a href="#" data-action="navAndToggleDropdown" data-page="messages">Nachrichten</a>
    ...
    actions.innerHTML = `
      <button class="btn btn-outline" data-action="nav" data-page="login">Anmelden</button>
      <button class="btn btn-primary" data-action="nav" data-page="register">Registrieren</button>`;
  }
  ...
  if (mobileMenu) {
    if (state.user) {
      mobileMenu.innerHTML = `
        <a href="#" data-action="navAndClose" data-page="jobs">Jobs finden</a>
        <a href="#" data-action="navAndClose" data-page="employer-landing">Für Arbeitgeber</a>
        <a href="#" data-action="logoutAndClose" style="color:var(--gray-500)">Abmelden</a>`;
    } else {
      mobileMenu.innerHTML = `
        <a href="#" data-action="navAndClose" data-page="jobs">Jobs finden</a>
        <a href="#" data-action="navAndClose" data-page="employer-landing">Für Arbeitgeber</a>
        <a href="#" data-action="navAndClose" data-page="login">Anmelden</a>
        <a href="#" data-action="navAndClose" data-page="register">Registrieren</a>`;
    }
  }
}
...
function showShareMenu(url, text) {
  ...
  const html = `
    <div id="share-menu" role="dialog" aria-modal="true" aria-label="Job teilen" style="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9998;display:flex;align-items:center;justify-content:center;padding:1rem" data-action="closeOverlay">
      <div style="background:#fff;border-radius:12px;padding:1.25rem;max-width:360px;width:100%;box-shadow:0 20px 60px rgba(0,0,0,0.25)">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem">
          ...
          <button class="close-btn" data-action="closeShareMenu" aria-label="Schließen">&times;</button>
        </div>
        <div style="display:flex;flex-direction:column;gap:0.5rem">
          ...
          <button class="btn btn-block btn-outline" data-action="copyJobLink" data-url="${escapeAttr(url)}" style="display:flex;align-items:center;justify-content:center;gap:0.5rem">
...
function renderChatWidget() {
  ...
  if (!state.activeChat) {
    content.innerHTML = `
      <div class="chat-list">
        ${chatList.map(c => `
          <div class="chat-list-item" data-action="openChat" data-chat-id="${c.id}">
    ...
    content.innerHTML = `
      <div style="padding:0.5rem;border-bottom:1px solid var(--gray-200);display:flex;align-items:center;gap:0.5rem">
        <button data-action="closeChatDetail" style="background:none;border:none;cursor:pointer;font-size:1.1rem">&#8592;</button>
      ...
      <div class="chat-input-area">
        ...
        <input type="text" placeholder="Nachricht..." id="chat-input" data-on-keydown="chatInputEnter" aria-label="Nachricht schreiben">
        <button data-action="sendChatMessage">→</button>
      </div>`;
  }
}
    ...
    <div class="employer-hero" style="min-height:560px">
      ...
      <div class="employer-hero-content slide-up" style="max-width:700px;padding:4rem 2rem">
        ...
        <div style="display:flex;gap:1rem;flex-wrap:wrap">
          <button class="btn btn-lg" style="background:#fff;color:#1d4ed8;font-size:1rem;padding:0.875rem 2.5rem" data-action="nav" data-page="jobs">Jobs entdecken</button>
          <button class="btn btn-lg" style="background:transparent;border:2px solid rgba(255,255,255,0.4);color:#fff;font-size:1rem;padding:0.875rem 2rem" data-action="nav" data-page="employer-landing">Für Arbeitgeber</button>
        </div>
      </div>
    </div>
    ...
    <section class="cta-fullblock">
      <div class="cta-fullblock-inner">
        ...
        <div class="cta-fullblock-buttons">
          <button class="btn btn-lg cta-btn-primary" data-action="nav" data-page="jobs">Jobs entdecken <span class="cta-arrow">→</span></button>
          <button class="btn btn-lg cta-btn-outline" data-action="nav" data-page="employer-landing">Für Arbeitgeber</button>
        </div>
      </div>
    </section>`;
}
    ...
    <div class="page page-wide">
      ...
      <div class="search-header">
        <div class="search-bar">
          ...
          <input type="text" id="job-search-input" placeholder="Job, Unternehmen oder Stichwort..." value="${state.filters.search}" data-on-input="searchFilter" aria-label="Nach Jobs suchen">
          <button class="btn btn-primary" data-action="render">Suchen</button>
        </div>
      ...
      ${(() => {
        ...
        return `
        <button class="mobile-filter-toggle" data-action="toggleMobileFilters" aria-expanded="${isOpen}" aria-controls="search-sidebar">
      ...
      <div class="search-layout ${state.mobileFiltersOpen ? 'show-mobile-filters' : ''}">
        <aside class="search-sidebar" id="search-sidebar">
          ...
          <div class="filter-section">
            ...
            <input type="text" class="form-input" id="address-filter-input" placeholder="Straße + PLZ + Stadt (z.B. Hauptstr. 1, 40213 Düsseldorf)" value="${state.filters.address || ''}" aria-labelledby="addr-filter-heading" data-on-input="addressFilter" data-on-keydown="addressFilterEnter">
          ...
          <div class="filter-section">
            <h4>Stadt</h4>
            <select class="form-select" data-on-change="setFilterCity">
          ...
          <div class="filter-section">
            ...
            <div style="display:flex;flex-wrap:wrap;gap:0.4rem">
              ${[6,7,8,9,10,11,12].map(h => `<label style="display:flex;align-items:center;gap:0.3rem;font-size:0.85rem;cursor:pointer">
                <input type="checkbox" ${state.filters.hours.includes(h)?'checked':''} data-on-change="toggleHoursFilter" data-hours="${h}"> ${h}
              </label>`).join('')}
            </div>
          </div>
          ...
          <div class="filter-section">
            ...
            <input type="range" class="range-slider" min="5" max="50" step="5" value="${state.filters.radius}" data-on-input="setFilterRadius">
          ...
          <button class="btn btn-primary btn-block" data-action="applyFilters">Filter anwenden</button>
          <button class="btn btn-ghost btn-block" style="font-size:0.8rem;color:var(--gray-500);margin-top:0.25rem" data-action="resetFilters">Filter zurücksetzen</button>
        </aside>
...
function renderJobCard(j) {
  ...
  return `
    <div class="job-card ${j.promoted ? 'promoted' : ''}" data-action="navToJob" data-job-id="${j.id}">
      ...
      <div class="job-card-actions">
        ...
        <button class="save-btn ${saved ? 'saved' : ''}" data-action="toggleSaveJob" data-job-id="${j.id}" title="${saved ? 'Gespeichert' : 'Speichern'}" aria-label="${saved ? 'Job entfernen' : 'Job speichern'}" aria-pressed="${saved}">
...
function renderJobDetail() {
  ...
  return `
    <div class="page">
      <button class="btn btn-ghost mb-2" data-action="nav" data-page="jobs">&#8592; Zurück zur Suche</button>
          ...
          <div class="job-detail-header">
            <div style="display:flex;justify-content:space-between;align-items:start">
              ...
              <div style="display:flex;gap:0.5rem;align-items:center">
                <button class="save-btn" data-action="shareJob" data-job-id="${job.id}" style="font-size:1.25rem;background:none;border:none;cursor:pointer;padding:0.35rem" aria-label="Job teilen" title="Job teilen">
                ...
                <button class="save-btn ${saved ? 'saved' : ''}" data-action="toggleSaveJob" data-job-id="${job.id}" style="font-size:1.5rem" aria-label="${saved ? 'Job entfernen' : 'Job speichern'}" aria-pressed="${saved}"><span aria-hidden="true">${saved ? '♥' : '♡'}</span></button>
              </div>
            </div>
            ...
            <div class="card-body">
              ${(() => {
                ...
                const label =
                  ...
                  'Jetzt bewerben';
                const dataAction = !state.user
                  ? 'data-action="nav" data-page="login"'
                  : `data-action="submitApplication" data-job-id="${job.id}"`;
                return `<button class="btn btn-primary btn-block btn-lg" id="apply-btn-${job.id}" ${dataAction} ${disabled ? 'disabled style="opacity:0.6;cursor:not-allowed"' : ''}>${label}</button>`;
              })()}
      ...
      <div class="dashboard-layout">
        ...
        <div class="dashboard-content">
            ...
            <div class="card" style="margin-bottom:1.5rem;border:2px solid var(--success);background:linear-gradient(135deg,#f0fdf4,#dcfce7)">
              <div class="card-body" style="display:flex;align-items:center;justify-content:space-between;gap:1rem;flex-wrap:wrap">
                ...
                <div style="display:flex;gap:0.5rem;flex-shrink:0">
                  ${job ? `<button class="btn btn-sm btn-outline" data-action="navToJob" data-job-id="${job.id}">Details</button>` : ''}
                  <button class="btn btn-sm btn-primary" style="background:var(--danger);border-color:var(--danger)" data-action="confirmEndActiveJob">Job beenden</button>
                </div>
              </div>
            </div>`;
          })()}
          ...
          <div class="worker-dash-grid">
            ...
            <!-- Nachrichten -->
            <div class="worker-dash-tile" data-action="nav" data-page="messages">
            ...
            <!-- Gespeicherte Jobs -->
            <div class="worker-dash-tile" data-action="nav" data-page="saved-jobs">
              <div class="wdt-header">
              ...
              ${saved.length > 0 ? saved.map(j => `
                <div class="wdt-job-row" data-action="navToJob" data-job-id="${j.id}">
            ...
            <div class="worker-dash-tile">
              ...
              <div style="display:flex;gap:0.5rem;flex-direction:column;margin-top:0.75rem">
                <button class="btn btn-primary btn-sm" data-action="nav" data-page="cv-builder">
                ...
                <label class="btn btn-outline btn-sm" style="cursor:pointer;text-align:center">
                  PDF hochladen
                  <input type="file" accept=".pdf,.doc,.docx" style="display:none" data-on-change="handleUploadCV">
                </label>
              </div>
            </div>
          ...
          <div class="profile-progress-card">
            ...
            <div class="pprogress-steps-row">
              ${todo.map(s => `
                <div class="pprogress-step-pill" ${s.page ? `data-action="navToSection" data-page="${s.page}" data-section="${s.section||''}"` : ''}>
  ...
  return `
    <div class="dashboard-sidebar">
      <nav class="sidebar-nav">
        <a href="#" class="${active==='dashboard'?'active':''}" data-action="nav" data-page="worker-dashboard">Dashboard</a>
        <a href="#" class="${active==='profile-view'?'active':''}" data-action="nav" data-page="worker-profile-view">Mein Profil</a>
        <a href="#" class="${active==='saved'?'active':''}" data-action="nav" data-page="saved-jobs">Gespeicherte Jobs</a>
      ...
      <nav class="sidebar-nav">
        <a href="#" class="${active==='support'?'active':''}" data-action="nav" data-page="support" style="font-size:0.82rem;opacity:0.6">Support</a>
      </nav>
    </div>`;
}
...
function renderWorkerProfileView() {
  ...
  const missingTag = (label, step) =>
    `<span class="pv-missing-chip" data-action="gotoProfileAndNav" data-step="${step}">+ ${label} hinzufügen</span>`;
      ...
      <div class="dashboard-layout">
        ...
        <div class="dashboard-content">
          ...
          <div class="pv-banner">
            ...
            <button class="btn btn-sm btn-outline" style="margin-left:auto" data-action="gotoProfileStep" data-step="0" data-then-nav="worker-profile">
              Profil bearbeiten
            </button>
          </div>
                ...
                <div class="epc-doc-row ${u.cvUploaded ? 'ok' : 'missing'}">
                  ${u.cvUploaded
                    ...
                    : `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="var(--gray-300)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                       ...
                       <button class="pv-edit-btn" data-action="gotoProfileStep" data-step="1" data-then-nav="worker-profile">Hochladen</button>`}
                </div>
                <div class="epc-doc-row ${u.docsUploaded ? 'ok' : 'missing'}">
                  ${u.docsUploaded
                    ...
                    : `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="var(--gray-300)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                       ...
                       <button class="pv-edit-btn" data-action="gotoProfileStep" data-step="2" data-then-nav="worker-profile">Hochladen</button>`}
                </div>
              </div>
            </div>
  ...
  const stepContent = [
    ...
    ${u.cvUploaded ? `
      <div class="upload-success-bar">
        ...
        <button class="btn btn-sm btn-outline" style="margin-left:auto" data-action="clearUserFlag">Entfernen</button>
      </div>
      <p style="margin-top:1rem;font-size:0.85rem;color:var(--gray-500);text-align:center">
        <a href="#" data-action="nav" data-page="cv-builder" style="color:var(--primary);font-weight:600">Mit dem Builder bearbeiten</a>
      </p>` : `
      <div class="cv-upload-area">
        ...
        <div class="cv-upload-option">
          <button class="cv-upload-btn" data-action="nav" data-page="cv-builder">
      ...
      <div class="cv-upload-area">
        <div class="cv-upload-option">
          <label class="cv-upload-btn" style="cursor:pointer">
            ...
            <input type="file" accept=".pdf,.jpg,.jpeg,.png" multiple style="display:none" data-on-change="handleDocScanned">
          </label>
        </div>
        <div class="cv-upload-divider">oder</div>
        <div class="cv-upload-option">
          <label class="cv-upload-btn" style="cursor:pointer">
            ...
            <input type="file" accept="image/*" capture="environment" style="display:none" data-on-change="handleDocScanned">
          </label>
        </div>
      </div>`}`,
    ...
    <div class="form-group">
      ...
      <div class="checkbox-group">
        ${SKILLS.map(s => `<label class="checkbox-label"><input type="checkbox" ${u.skills?.includes(s)?'checked':''} data-on-change="limitSkills"> <span>${s}</span></label>`).join('')}
      </div>
    </div>
      ...
      <div id="refs-list">
        ${(u.refs||[]).map((r,i) => `
          <div style="display:flex;align-items:center;gap:0.5rem;padding:0.55rem 0.75rem;background:var(--gray-50);border-radius:8px;margin-bottom:0.4rem;font-size:0.88rem">
            ...
            <button class="btn btn-sm btn-ghost" data-action="removeRef" data-index="${i}">✕</button>
          </div>`).join('')}
      </div>
      <button class="btn btn-outline btn-sm" data-action="addRef">+ Referenz hinzufügen</button>
    </div>
    <div class="form-group" style="margin-top:1.25rem">
        ...
        <div class="dashboard-content">
          ...
          <div class="profile-step-indicators">
            ${PROFILE_STEPS.map((s,i) => `
              <div class="psi-item ${i===step?'active':''} ${i<step?'done':''}" data-action="gotoProfileStep" data-step="${i}">
                <div class="psi-dot">
                  ${i < step
          ...
          <div class="profile-step-nav">
            ${step > 0
              ? `<button class="btn btn-outline" data-action="prevProfileStep">← Zurück</button>`
              : `<button class="btn btn-outline" data-action="nav" data-page="worker-dashboard">← Dashboard</button>`}
            ${step < total-1
              ? `<button class="btn btn-primary" data-action="nextProfileStep">Weiter →</button>`
              : `<button class="btn btn-primary" data-action="finishProfileSetup">✓ Profil speichern</button>`}
          </div>
        ...
        <div class="dashboard-content">
          ...
          ` : `
            <div class="empty-state">
              ...
              <button class="btn btn-primary" data-action="nav" data-page="jobs">Jobs entdecken</button>
            </div>
          `}
        </div>
      </div>
    ...
    <div class="page">
      <div class="dashboard-layout">
        ...
        <div class="dashboard-content">
          ...
          <div class="jobs-grid">
            ${myApps.map(a => `
              <div class="card" style="cursor:pointer" data-action="navToJob" data-job-id="${a.job.id}">
          ...
          <div class="empty-state" style="margin-top:2rem">
            ...
            <button class="btn btn-primary" data-action="nav" data-page="jobs">Jobs finden</button>
          </div>`}
        </div>
      </div>
    </div>`;
        ...
        <div class="dashboard-content">
          ...
          <div style="margin-bottom:1.5rem">
            ...
            <div class="cv-templates">
              <div class="cv-template selected" data-template="modern" data-action="selectCvTemplate">
              ...
              <div class="cv-template" data-template="classic" data-action="selectCvTemplate">
              ...
              <div class="cv-template" data-template="creative" data-action="selectCvTemplate">
                <div class="cv-preview" style="overflow:hidden;font-family:Inter,sans-serif;padding:0">
          ...
          <div class="card">
            ...
            <div class="card-body">
              ...
              <div style="display:flex;align-items:center;gap:1.25rem;margin-bottom:1.5rem;padding-bottom:1.5rem;border-bottom:1px solid var(--gray-200)">
                <div id="cv-photo-preview" data-action="triggerFileInput" data-target-id="cv-photo-input" style="width:80px;height:80px;border-radius:50%;background:var(--gray-100);display:flex;align-items:center;justify-content:center;cursor:pointer;border:2px dashed var(--gray-300);overflow:hidden;flex-shrink:0;transition:all 0.2s;${state.cvPhoto ? 'background-image:url('+state.cvPhoto+');background-size:cover;background-position:center;border-style:solid;border-color:var(--primary)' : ''}">${state.cvPhoto ? '' : '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="var(--gray-400)" stroke-width="2"><path d="M23 19a2 2 0 01-2 2H3a2 2 0 01-2-2V8a2 2 0 012-2h4l2-3h6l2 3h4a2 2 0 012 2z"/><circle cx="12" cy="13" r="4"/></svg>'}</div>
                <input type="file" id="cv-photo-input" accept="image/*" style="display:none" data-on-change="handleCVPhoto">
                <div>
            ...
            <div class="card-footer" style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:0.5rem">
              <button class="btn btn-outline" data-action="previewCV">Vorschau anzeigen</button>
              <div style="display:flex;gap:0.5rem">
                <button class="btn btn-outline" data-action="downloadCV">
                ...
                <button class="btn btn-primary" data-action="addCVToProfile">
...
function renderEmployerLanding() {
  return `
    <!-- Hero: blau, linksbündig, weniger perfekt = weniger KI -->
    <div style="background:#1e3a8a;padding:4.5rem 1.5rem 4rem;color:#fff">
      <div style="max-width:1060px;margin:0 auto">
        <p style="font-size:0.78rem;letter-spacing:0.12em;text-transform:uppercase;color:rgba(255,255,255,0.55);margin:0 0 1.5rem;font-weight:600">WorkPilot für Arbeitgeber</p>
        <h1 style="font-size:clamp(2rem,4.5vw,3.2rem);line-height:1.1;font-weight:800;margin:0 0 1.25rem;max-width:580px;color:#fff">Junge Aushilfen finden. Ohne Aufwand.</h1>
        <p style="font-size:1.05rem;color:rgba(255,255,255,0.7);margin:0 0 2rem;line-height:1.6;max-width:440px">Schreib eine Anzeige, bekomm Bewerbungen, kläre alles im Chat. Kostenlos.</p>
        <button class="btn btn-lg" data-action="goPostJob" style="background:#fff;color:#1e3a8a;font-weight:700;padding:0.85rem 2rem;border-radius:8px;border:none;font-size:0.95rem;cursor:pointer">Anzeige schalten</button>
      </div>
    </div>

    <!-- Slideshow: So geht's - mit Fotos -->
    <div style="padding:4rem 0;background:#fff">
      <div style="max-width:1060px;margin:0 auto;padding:0 1.5rem">
        <h2 style="font-size:1.4rem;font-weight:700;margin:0 0 1.75rem;color:var(--gray-900)">So geht's</h2>
      </div>
          ...
          <div class="emp-slideshow-slides" id="emp-slides">
            <div class="emp-slideshow-slide">
              ...
              <div class="emp-slideshow-caption">
                ...
                <p>Titel, Stundenlohn, Standort. Fertig in fünf Minuten.</p>
              </div>
            </div>
            <div class="emp-slideshow-slide">
              ...
              <div class="emp-slideshow-caption">
                ...
                <p>Profil, Lebenslauf, Verfügbarkeit. Alles im Dashboard.</p>
              </div>
            </div>
            <div class="emp-slideshow-slide">
              ...
              <div class="emp-slideshow-caption">
                ...
                <p>Termine und Rückfragen direkt per Nachricht.</p>
              </div>
            </div>
          </div>
    ...
    <!-- Vorteile: linksbündig, kein Icon-Raster, mehr wie ein Artikel -->
    <div style="padding:4rem 1.5rem;background:var(--gray-50)">
      <div style="max-width:1060px;margin:0 auto">
        <h2 style="font-size:1.4rem;font-weight:700;margin:0 0 2rem;color:var(--gray-900)">Warum WorkPilot</h2>
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:2rem">
          <div>
            <h3 style="font-size:1rem;font-weight:700;margin:0 0 0.4rem;color:var(--gray-900)">Motivierte Bewerber</h3>
            <p style="font-size:0.88rem;color:var(--gray-500);margin:0;line-height:1.55">Schüler und Studenten, die wirklich arbeiten wollen. Mit Profil und Lebenslauf.</p>
          </div>
          <div>
            <h3 style="font-size:1rem;font-weight:700;margin:0 0 0.4rem;color:var(--gray-900)">Chat statt E-Mail</h3>
            <p style="font-size:0.88rem;color:var(--gray-500);margin:0;line-height:1.55">Termine, Fragen, Zusagen. Alles im Messenger, nicht im Postfach.</p>
          </div>
          <div>
            <h3 style="font-size:1rem;font-weight:700;margin:0 0 0.4rem;color:var(--gray-900)">Kostenlos</h3>
            <p style="font-size:0.88rem;color:var(--gray-500);margin:0;line-height:1.55">Kein Abo, kein Vertrag, keine Provision. Auch nicht irgendwann.</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <!-- Testimonials -->
    <div style="padding:4rem 0;overflow:hidden;background:#fff">
      <div style="max-width:1060px;margin:0 auto;padding:0 1.5rem">
        <h2 style="font-size:1.4rem;font-weight:700;margin:0 0 2rem;color:var(--gray-900)">Das sagen Arbeitgeber</h2>
      </div>
      <div class="testi-ticker-wrap">
        <div class="testi-ticker-fade testi-ticker-fade-left"></div>
        <div class="testi-ticker-fade testi-ticker-fade-right"></div>
        <div class="testi-ticker">
          <div class="testi-ticker-track">
            ${(() => {
              var list = [
                { name:'Sandra K.', role:'Café Sonnenschein, Köln', photo:'https://randomuser.me/api/portraits/women/65.jpg', text:'\u201EInnerhalb von einem Tag vier Bewerbungen. Eine arbeitet seit drei Monaten bei mir.\u201C', rating:5 },
                { name:'Thomas M.', role:'MediaMarkt, Düsseldorf', photo:'https://randomuser.me/api/portraits/men/52.jpg', text:'\u201EHingucken, anschreiben, fertig. Ohne Papierkram.\u201C', rating:5 },
                { name:'Lara H.', role:'TechStart, Berlin', photo:'https://randomuser.me/api/portraits/women/82.jpg', text:'\u201EDer Chat ist Gold wert. Schüler antworten schneller als Erwachsene.\u201C', rating:4.5 },
                { name:'Markus R.', role:'Bäckerei Richter, München', photo:'https://randomuser.me/api/portraits/men/45.jpg', text:'\u201EEine Woche, alle Stellen besetzt.\u201C', rating:5 },
                { name:'Anna B.', role:'Buchhandlung Blatt, Hamburg', photo:'https://randomuser.me/api/portraits/women/33.jpg', text:'\u201EEndlich was für junge Aushilfen.\u201C', rating:5 },
                { name:'Felix W.', role:'Pizzeria Napoli, Stuttgart', photo:'https://randomuser.me/api/portraits/men/67.jpg', text:'\u201EKostenlos. Funktioniert. Fertig.\u201C', rating:4.5 },
              ];
              var render = function(t) {
                var pct = (t.rating / 5) * 100;
    ...
    <!-- CTA: schlicht -->
    <div style="background:#1e3a8a;padding:3.5rem 1.5rem;text-align:center">
      <p style="color:rgba(255,255,255,0.55);font-size:0.82rem;text-transform:uppercase;letter-spacing:0.1em;font-weight:600;margin:0 0 0.75rem">Bereit?</p>
      <h2 style="color:#fff;font-size:1.6rem;font-weight:700;margin:0 0 0.6rem">Jetzt Anzeige schalten.</h2>
      <p style="color:rgba(255,255,255,0.6);font-size:0.92rem;margin:0 0 1.5rem">Fünf Minuten. Kostenlos.</p>
      <button class="btn btn-lg" data-action="goPostJob" style="background:#fff;color:#1e3a8a;font-weight:700;padding:0.85rem 2rem;border-radius:8px;border:none;font-size:0.95rem;cursor:pointer">Loslegen</button>
    </div>`;
}
      ...
      <div class="dashboard-layout">
        ...
        <div class="dashboard-content">
          <div class="employer-dash-header">
            ...
            <button class="btn btn-employer" data-action="goPostJob" ${!state.user.approved ? 'disabled style="opacity:0.5;cursor:not-allowed"' : ''}>+ Neue Anzeige schalten</button>
          </div>
              ...
              <div class="card">
                <div class="card-body" style="display:flex;justify-content:space-between;align-items:center;gap:1rem">
                  ...
                  <div style="display:flex;align-items:center;gap:0.5rem">
                    ...
                    <button class="btn btn-sm" style="background:var(--danger);color:#fff;font-size:0.75rem" data-action="deletePostedJob" data-job-id="${j.id}">Löschen</button>
                  </div>
                </div>
              </div>
            `).join('')}
          ...
          <div class="empty-state" style="margin-top:2rem">
            ...
            <button class="btn btn-employer" data-action="goPostJob">+ Erste Anzeige erstellen</button>
          </div>`}
        </div>
      </div>
    </div>`;
...
function renderPostJob() {
  ...
  if (!state.user.approved) {
    return `<div class="page page-narrow" style="text-align:center;padding-top:4rem">
      ...
      <button class="btn btn-primary" style="margin-top:1.5rem" data-action="nav" data-page="employer-dashboard">Zurück zum Dashboard</button>
    </div>`;
  }
  const step = state.wizardStep;
  ...
  return `
    ...
    ${imgs.length > 0 ? `
      ...
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:0.75rem;margin-bottom:1.5rem">
        ${imgs.map((img, i) => {
          ...
          return `
          <button type="button" data-action="toggleJobImage" data-index="${i}" aria-pressed="${selected}" aria-label="Bild ${i+1}${selected ? ' — ausgewählt' : ''}" style="position:relative;aspect-ratio:1;border-radius:var(--radius-sm);overflow:hidden;cursor:pointer;padding:0;background:none;border:3px solid ${selected ? 'var(--primary)' : 'transparent'}">
    ...
    ` : `
      <div class="empty-state" style="padding:2rem">
        ...
        <button class="btn btn-outline btn-sm" data-action="nav" data-page="employer-profile">Zum Profil</button>
      </div>
    `}`;
}
      ...
      <div class="dashboard-layout">
        ...
        <div class="dashboard-content">
              ...
              <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:0.75rem;margin-bottom:1rem" id="company-images-grid">
                ${(state.user.companyImages || []).map((img, i) => `
                  <div style="position:relative;aspect-ratio:1;border-radius:var(--radius-sm);overflow:hidden">
                    ...
                    <button data-action="removeCompanyImage" data-index="${i}" aria-label="Bild ${i+1} entfernen" style="position:absolute;top:4px;right:4px;width:24px;height:24px;border-radius:50%;background:rgba(0,0,0,0.6);color:#fff;border:none;cursor:pointer;font-size:0.8rem;display:flex;align-items:center;justify-content:center">&#10005;</button>
                  </div>
                `).join('')}
                ${(state.user.companyImages || []).length < 6 ? `
                <div style="aspect-ratio:1;border:2px dashed var(--gray-300);border-radius:var(--radius-sm);display:flex;flex-direction:column;align-items:center;justify-content:center;cursor:pointer;color:var(--gray-500);font-size:0.85rem" data-action="triggerFileInput" data-target-id="company-images-input">
                  <div style="font-size:1.5rem">+</div>
                  Bild hinzufügen
                </div>` : ''}
              </div>
              <input type="file" id="company-images-input" accept="image/*" style="display:none" data-on-change="handleCompanyImage">
            </div>
          ...
          <div style="margin-top:1.5rem;text-align:right">
            <button class="btn btn-primary btn-lg" data-action="saveEmployerProfile">✓ Profil speichern</button>
          </div>
        </div>
      </div>
    </div>`;
    ...
    <div class="page">
      <div class="dashboard-layout">
        ...
        <div class="dashboard-content">
          ...
          <div class="tabs">
            <button class="tab ${!state.applicantFilter || state.applicantFilter==='all' ? 'active' : ''}" data-action="setApplicantFilter" data-filter="all">Alle (${allApplicants.length})</button>
            <button class="tab ${state.applicantFilter==='new' ? 'active' : ''}" data-action="setApplicantFilter" data-filter="new">Neu (${allApplicants.filter(a=>a.status==='new').length})</button>
            <button class="tab ${state.applicantFilter==='reviewing' ? 'active' : ''}" data-action="setApplicantFilter" data-filter="reviewing">In Prüfung (${allApplicants.filter(a=>a.status==='reviewing').length})</button>
                  ...
                  <tr${workerHasActiveJob ? ' style="opacity:0.6"' : ''}>
                    ...
                    <td>
                      <div style="display:flex;gap:0.375rem;flex-wrap:wrap">
                        <button class="btn btn-sm btn-outline" data-action="navToApplicant" data-applicant-id="${a.id}">Profil</button>
                        ${a.motivation || a.motivationFileName ? '<button class="btn btn-sm btn-outline" data-action="openApplicationDoc" data-app-id="' + a.id + '" data-doc-type="motivation" title="Motivationsschreiben öffnen" style="color:#2563eb;border-color:#2563eb">Anschreiben</button>' : ''}
                        <button class="btn btn-sm btn-outline" data-action="openApplicationDoc" data-app-id="${a.id}" data-doc-type="cv" title="Lebenslauf öffnen" style="color:#6366f1;border-color:#6366f1">Lebenslauf</button>
                        <button class="btn btn-sm btn-primary" data-action="openApplicantChat" data-app-id="${a.id}">Nachricht</button>
                      </div>
                    </td>
                  </tr>`;
                }).join('')}
      ...
      <div class="dashboard-layout">
        ...
        <div class="dashboard-content">
          ...
          <div style="display:flex;align-items:center;gap:0.75rem;margin-bottom:1.5rem">
            <button class="btn btn-sm btn-outline" data-action="nav" data-page="applicants">&#8592; Zurück</button>
          ...
          <div class="card" style="border-radius:0 0 12px 12px;padding-top:2.5rem;margin-bottom:1rem">
            <div class="card-body">
              <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:0.75rem">
                ...
                <button class="btn btn-primary" data-action="openApplicantChat" data-app-id="${a.id}">
          ...
          <div style="text-align:center;margin-top:1.5rem">
            <button class="btn btn-primary btn-lg" data-action="openApplicantChat" data-app-id="${a.id}">
        ...
        <div class="dashboard-content">
          ...
          <div class="card" style="padding:1.5rem;margin-bottom:2rem">
            ...
            <div style="display:flex;flex-direction:column;gap:0.75rem">
              ...
              <button class="btn btn-primary" data-action="submitSupportTicket" style="align-self:flex-start">Ticket absenden</button>
            </div>
          </div>
    ...
    <div class="page">
      <div class="dashboard-layout">
        ...
        <div class="dashboard-content">
          ...
          <div class="tabs">
            <button class="tab ${!state.reviewTab || state.reviewTab==='received' ? 'active' : ''}" data-action="setReviewTab" data-tab="received">Erhaltene Bewertungen</button>
            <button class="tab ${state.reviewTab==='given' ? 'active' : ''}" data-action="setReviewTab" data-tab="given">Abgegebene Bewertungen</button>
          </div>
          ...
          ${(() => {
            ...
            <div class="card" style="margin-top:1.5rem">
              ...
              <div class="card-body">
                ...
                <button class="btn btn-primary" data-action="submitReview">Bewertung absenden</button>
              </div>
            </div>`;
          })()}
        </div>
...
function renderAGB() {
  return `<div class="page page-narrow" style="padding-top:3rem">
    ...
    <div style="background:#fff;border:1px solid var(--gray-200);border-radius:var(--radius-lg);padding:2rem;margin-bottom:1.5rem">
      ...
      <p style="color:var(--gray-600);line-height:1.8;font-size:0.9rem">Der Schutz personenbezogener Daten ist uns wichtig. Einzelheiten zur Verarbeitung personenbezogener Daten entnehmen Sie bitte unserer <a href="#" data-action="nav" data-page="datenschutz" style="color:var(--primary);font-weight:600">Datenschutzerklärung</a>.</p>
    </div>
    ...
    <div class="page-wide admin-panel" style="padding-top:2rem">
      <div class="admin-header">
        ...
        <div style="display:flex;gap:0.5rem;align-items:center">
          ...
          <button class="btn btn-outline" data-action="nav" data-page="admin-panel" style="font-size:0.85rem;margin-left:0.75rem"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="vertical-align:-2px"><path d="M23 4v6h-6M1 20v-6h6"/><path d="M3.51 9a9 9 0 0114.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0020.49 15"/></svg> Aktualisieren</button>
          <button class="btn btn-ghost" data-action="adminLogout" style="color:var(--danger);font-size:0.85rem">Abmelden</button>
        </div>
      </div>
                ...
                ${pending.map(u => {
                  ...
                  return `<tr class="admin-table-row" style="border-bottom:1px solid var(--gray-100)">
                    ...
                    <td style="padding:0.8rem 1.25rem;text-align:center;white-space:nowrap">
                      <button class="btn btn-sm" style="background:var(--success);color:#fff;font-size:0.78rem;font-weight:600;padding:0.45rem 0.9rem;margin-right:0.3rem" data-action="adminToggleApproval" data-user-id="${u.id}" data-approve="true">✓ Freischalten</button>
                      <button class="btn btn-sm btn-ghost" style="font-size:0.78rem;color:var(--danger)" data-action="adminRemoveEmployer" data-user-id="${u.id}">Ablehnen</button>
                    </td>
                  </tr>`;
                }).join('')}
              </tbody>
      ...
      <div class="admin-tabs">
        <button class="admin-tab ${!state.adminTab || state.adminTab === 'besucher' ? 'active' : ''}" data-action="switchAdminTab" data-tab="besucher">
        ...
        <button class="admin-tab ${state.adminTab === 'support' ? 'active' : ''}" data-action="switchAdminTab" data-tab="support">
      ...
      <div class="card admin-chart-card" style="margin-bottom:1.5rem"><div class="card-body" style="padding:0;overflow:hidden">
        <div style="padding:1.25rem 1.25rem 0.75rem;display:flex;align-items:center;gap:0.5rem">
          ...
          <button class="btn btn-sm btn-outline" style="margin-left:0.5rem;font-size:0.75rem" data-action="adminRefreshProfiles">Neu laden</button>
        </div>
            ...
            ${employers.map(u => {
              ...
              return `<tr class="admin-table-row">
                ...
                <td style="padding:0.65rem 1.25rem;text-align:center;white-space:nowrap">
                  ${u.approved !== false
                    ? `<button class="btn btn-sm" style="background:#f97316;color:#fff;font-size:0.75rem;margin-right:0.25rem" data-action="adminToggleApproval" data-user-id="${u.id}" data-approve="false">Sperren</button>`
                    : `<button class="btn btn-sm" style="background:var(--success);color:#fff;font-size:0.75rem;margin-right:0.25rem" data-action="adminToggleApproval" data-user-id="${u.id}" data-approve="true">Entsperren</button>`}
                  <button class="btn btn-sm" style="background:var(--danger);color:#fff;font-size:0.75rem" data-action="adminRemoveEmployer" data-user-id="${u.id}">Entfernen</button>
                </td>
              </tr>`;
          ...
          <tbody>
            ${(state._allProfilesCache || []).map(u => {
              ...
              const actionBtn = u.role === 'employer'
                ? (u.approved
                    ? `<button class="btn btn-sm" style="background:var(--danger);color:#fff;font-size:0.75rem" data-action="adminToggleApproval" data-user-id="${u.id}" data-approve="false">Sperren</button>`
                    : `<button class="btn btn-sm" style="background:var(--success);color:#fff;font-size:0.75rem" data-action="adminToggleApproval" data-user-id="${u.id}" data-approve="true">Freischalten</button>`)
                : '';
              return `<tr class="admin-table-row">
      ...
      <div style="text-align:center;padding:1.5rem 0 2.5rem;color:var(--gray-500);font-size:0.8rem">
        ...
        <a href="#" data-action="nav" data-page="admin-panel" style="color:var(--primary);text-decoration:none;font-weight:500">Jetzt aktualisieren</a>
      </div>
    </div>`;
}
...
if (typeof registerAction === 'function') {
  ...
  // ── Zusaetzliche Actions (aus Batch-Konvertierung) ──
  ...
  registerAction('closeOverlay', (el, e) => { if (e.target === el) el.remove(); });
  registerAction('closeShareMenu', () => { var m = document.getElementById('share-menu'); if (m) m.remove(); });
  registerAction('confirmEndActiveJob', () => { if (confirm('Möchtest du diesen Job wirklich beenden?')) endActiveJob(); });
  ...
  registerAction('removeRef', (el) => removeRef(parseInt(el.dataset.index)));
  registerAction('scrollToSection', (el) => { var t = document.getElementById(el.dataset.target); if (t) t.scrollIntoView({ behavior: 'smooth' }); });
  registerAction('setApplicantFilter', (el) => { state.applicantFilter = el.dataset.filter === 'all' ? null : el.dataset.filter; render(); });
  registerAction('setRating', (el) => setRating(parseInt(el.dataset.rating)));
  registerAction('setReviewTab', (el) => { state.reviewTab = el.dataset.tab; render(); });
  registerAction('toggleJobImage', (el) => toggleJobImage(parseInt(el.dataset.index)));
  registerAction('toggleTicketDetail', (el) => { var r = document.getElementById('ticket-detail-' + el.dataset.ticketId); if (r) r.style.display = r.style.display === 'none' ? 'table-row' : 'none'; });

  // ── Zusaetzliche Change-Actions ──
  registerChange('handleApplyCVFile', (el) => handleApplyCVFile(el));
  registerChange('handleApplyMotivationFile', (el) => handleApplyMotivationFile(el));
  registerChange('handleCVPhoto', (el) => handleCVPhoto(el));
  registerChange('handleCompanyImage', (el) => handleCompanyImage(el));
  registerChange('handleCompanyLogo', (el) => handleCompanyLogo(el));
