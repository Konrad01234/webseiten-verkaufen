# Boostwerk Köln – Website (schwarze Variante)
...
> **Kopie mit schwarzem Hintergrund.** Dieser Ordner ist eine 1:1-Kopie von
> `sites/boostwerk`. Einziger Unterschied: Der Hintergrund ist reines Schwarz
> (`#000000`) statt des bisherigen Nachtblaus (`#06070a`). Alle dunklen
> Flächen, Verläufe, Overlays und die Grafiken wurden von blaugrauen auf
> neutrale Grautöne umgestellt – Inhalte, Struktur und Animationen sind
> unverändert. Geändert wurden nur Farbwerte in `styles.css`, die
> `theme-color`-Angaben und `--logo-bg` in den HTML-Dateien, die Farben der
> SVG-Hebebühne sowie `img/favicon.svg`.
...
- **Stil:** dunkel, cineastisch – Schwarz (#000000) mit „Boost-Orange“ (#ff4d1c),
...
python3 tools/bilder.py ~/fotos/boostwerk --ziel sites/boostwerk-schwarz/img/fotos

# Erst mal nur schauen, was passieren würde
python3 tools/bilder.py ~/fotos --ziel sites/boostwerk-schwarz/img/fotos --probe

# Ein Bild als breites Foto-Band, mit festem Namen
python3 tools/bilder.py halle.jpg --ziel sites/boostwerk-schwarz/img \
...
Eigenes Projekt mit **Root Directory** `sites/boostwerk-schwarz`, Framework-Preset `Other`
(statisch, kein Build).
...
Das Skript liest die echten Seiten aus `sites/boostwerk/`
(für diese Variante entsprechend anpassen), wandelt die großen
